...
def main():
    ...
    if imp_rat:
        int_ip_gt = [var[0] for var in results]
        int_pred_gt = [var[1] for var in results]
        results = [(var[1], var[2], var[3], var[4]) for var in results]
        imp = [x-y for x,y in zip(int_pred_gt, int_ip_gt)] ## improvement of prediction from the input using gt mask 
        imp = sum(imp) 
        tot = sum(int_ip_gt)
        imp_ratio = imp/tot 
        print('improvement ratio: ', imp_ratio) 
        print('input*gt_class: ', sum(int_ip_gt)) 
        print('pred*gt_class: ', sum(int_pred_gt))
    ...
    if rank == 0:
        if args.out:
            ...
            # mmcv.dump(results, args.out)   # comment for trial 
        if args.eval:
